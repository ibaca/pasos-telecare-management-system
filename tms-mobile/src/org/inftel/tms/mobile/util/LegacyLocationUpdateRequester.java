...
public class LegacyLocationUpdateRequester extends LocationUpdateRequester {
	...
	public void requestLocationUpdates(long minTime, long minDistance, Criteria criteria,
		...
		// Note that we aren't monitoring this provider to check if it becomes disabled - this is
		// handled by the calling Activity.
	...
	public void requestPassiveLocationUpdates(long minTime, long minDistance,
			PendingIntent pendingIntent) {
		// Pre-Froyo there was no Passive Location Provider, so instead we will set an inexact
		// repeating, non-waking alarm
		// that will trigger once the minimum time between passive updates has expired. This is
		// potentially more expensive
		// than simple passive alarms, however the Receiver will ensure we've transitioned beyond
		...
		alarmManager.setInexactRepeating(AlarmManager.ELAPSED_REALTIME, System.currentTimeMillis()
				+ TmsConstants.MAX_TIME, TmsConstants.MAX_TIME, pendingIntent);
	}
}
